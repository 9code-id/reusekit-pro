...
extension DateTimeExtension on DateTime {
  ...
  String get dMMMykkss {
    return DateFormat('d MMM y kk:ss').format(toLocal().toLocal());
  }

  String get dMMMykkmmss {
    return DateFormat('d MMM y hh:mm:ss').format(toLocal().toLocal());
  }
  ...
  String get dEEE {
    return DateFormat('d EEEE').format(toLocal().toLocal());
  }
  ...
  String get yMdkk {
    return DateFormat('y-MM-dd kk').format(toLocal().toLocal());
  }

  String get yMdkkmm {
    return DateFormat('y-MM-dd kk:mm').format(toLocal().toLocal());
  }
  ...
  String get kkmmss {
    return DateFormat('hh:mm:ss').format(toLocal().toLocal());
  }
  ...
  bool get isYesterday {
    var date = this;
    var yesterday = DateTime(now.year, now.month, now.day, 0, 0)
  ...
  String get friendlyFormat {
    ...
    if (date.isToday) {
      ...
      return "Tomorrow, ${DateFormat('kk:mm').format(date)}";
    } else {
      if (this.year == now.year) {
        return DateFormat('d MMM, kk:mm').format(date);
      } else {
        return DateFormat('d MMM y, kk:mm').format(date);
      }
    }
  }
}
...
extension DateStringExtension on String {
  TimeOfDay get timeOfDay {
    //eg 12:00
    return TimeOfDay(
      hour: int.parse(split(":").first),
      minute: int.parse(split(":").last),
    );
  }
}
